...
public class Boot13JpaApplication {
	...
	public void init() {
		...
		Phone p1=Phone.builder().name("아이폰14").company("apple").price(150).build();
		Phone p2=Phone.builder().name("갤럭시S24").company("samsung").price(130).build();
		Phone p3=Phone.builder().name("아이폰15").company("apple").price(160).build();
		Phone p4=Phone.builder().name("갤럭시S25").company("samsung").price(170).build();
		//EntityManager 객체를 얻어내서
		...
		List<Phone> list = phoneRepo.findAll();
		for(Phone tmp:list) {
			System.out.println(tmp.getId()+"|"+tmp.getName()+"|"+tmp.getCompany());
		}
	
		List<PhoneDto> list2 = phoneRepo.findAll().stream().map(PhoneDto::toDto).toList();
		for(PhoneDto tmp:list2) {
	...
	public void initEmpDept() {
		...
		//DeptDtoRepository 객체가 리턴해주는 List<Dept> 를  이용해서 List<DeptDto> 객체 얻어내기
		List<DeptDto> deptList= deptRepo.findAll().stream().map(DeptDto::toDto).toList();
		for(DeptDto tmp:deptList) {
			//DeptDto 는 @Data 어노테이션이 붙어있는 클래스이므로 바로 출력해도 해당 객체안에 들어있는 정보를 볼 수 있다.
			System.out.println(tmp);
		}
