...
<head>
<meta charset="UTF-8">
<title>/views/user/pwd_updateform.jsp</title>
...
<body>
	<div class="container" id="app">
		...
		<form action="${pageContext.request.contextPath }/user/pwd_update" method="post" id="myForm">
			<div class="mb-2">
				<label class="form-label" for="pwd">기존 비밀번호</label>
				<input class="form-control" 
					@input="onPwdInput" 
					v-bind:class="{'is-invalid': !isPwdValid && isPwdDirty, 'is-valid':isPwdValid}" type="password" name="password" id="pwd"/>
				<div class="invalid-feedback">반드시 입력하세요</div>
			</div>
			<div class="mb-2">
				<label class="form-label" for="newPwd">새 비밀번호</label>
				<input class="form-control" type="password" name="newPassword" id="newPwd"
					@input="onNewPwdInput" 
					v-model="newPwd"
